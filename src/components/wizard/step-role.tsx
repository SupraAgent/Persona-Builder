...
export function StepRole({ draft, onChange }: Props) {
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-foreground">Role & Identity</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          Define who this persona is. Model them after a specific expert at a real company.
        </p>
      </div>

      <div className="space-y-4">
        <div>
          <label className="mb-1.5 block text-sm font-medium text-foreground">Persona Name</label>
          <Input
            value={draft.name}
        ...
        <div>
          ...
          <Input
            ...
            onChange={(e) => onChange({ yearsExperience: e.target.value ? Number(e.target.value) : null })}
            placeholder="e.g. 12"
        ...
        <div>
          <label className="mb-1.5 block text-sm font-medium text-foreground">Background Summary</label>
          <Textarea
